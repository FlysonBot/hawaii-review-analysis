...
As a heads up, it turns out the opposite actually happened, where higher business density reliably lead to lower rating.
...
**Dropping duplicate businesses.** `gmap_id` had fewer unique values than rows, meaning there's duplicate value. This is odd as it is the unique identifier of a business. I checked and found that some IDs appear exactly twice, so I compared the two rows of each of these IDs field by field, including the nested list and dictionary columns. Every duplicated pair turned out to be an exact copy of each other, which mean it's safe to keep only one of them. This is most likely an artifact of how the dataset was scraped.

**Extracting the zip code.** The dataset doesn't have a zip code column, so I extracted it out of the `address` column myself using regular expression, which is a powerful tool for specifying how to extract specific patterns from text. This took three attempts:

- My first pattern was just `(\d{5})`, which extract any five consecutive digits anywhere in the address. That produced obviously wrong zips like `15292`, which came from the address `Pele's Kitchen, 152929 Pahoa Village Rd, Pāhoa, HI 96778`, where the street number got captured instead of the zip.
- So I changed my pattern to `(\d{5})$` to specify it must be at the end of the address. That got 97.9% of non-missing addresses.
...
I checked that the remaining addresses really does not contain a zip code at all, so the third pattern correctly extract all zip-code that is extractable. Businesses with no address (1.48%) or no extractable zip (0.5% of the remaining) were dropped, since I cannot place them in a region and the whole question is about regions.

**Computing `zip_popularity`.** Once every business has a zip code, I group by zip code and count how many businesses are in each one, then assign that count back to every business in that zip. This is how I measure popularity of a region for my question.

**Cleaning `price`.** `price` is categorical variable and stored as values like `$$$` or `$$$$$`, so I converted it into character counts. `$` becomes 1, `$$$$` becomes 4. This keeps the ordering intact, which is better than encoding them as categorical for the later regression. Some of the rows used `₩` instead of `$`, and I simply treat them as the same as `$`.
...
I tried both of them, and it turns out the trend surprisingly reversed as shown in the unaggregated plot below.
...
Regardless of what the result shows, my hypothesis is definitely not supported by the evidence presented above. I cannot explain why the trend is negative instead of positive as predicted by my theory. I should not change my hypothesis because of this, as that would be cherry picking after seeing the actual trend in the dataset, which is methodologically non-legitimate. I'll use my original hypothesis in the hypothesis test later.

### Interesting Aggregates

The evidence above shows my hypothesis is wrong and there appears to be a negative correlation instead of a positive one. I would like to investigate whether this trend hold true when other variable change. I looked into how the columns correlate with each other. It turns out `avg_rating` is correlated with `price` and `log_num_of_reviews` on top of `log_zip_popularity`. Since `price` is missing for the majority of businesses, I'll look into whether the negative trend hold true across the different magnitudes of number of review instead.
...
|  | Q1 Rev | Q2 Rev | Q3 Rev | Q4 Rev |
| --- | --- | --- | --- | --- |
| **Q1 Pop** | 4.38 | 4.38 | 4.38 | 4.43 |
| **Q2 Pop** | 4.31 | 4.32 | 4.36 | 4.38 |
| **Q3 Pop** | 4.29 | 4.31 | 4.35 | 4.36 |
| **Q4 Pop** | 4.25 | 4.28 | 4.30 | 4.32 |

*(rows = zip popularity quartile, columns = number of reviews quartile)*

If we look down each column, the number is always decreasing. This indicate the negative correlation hold across different review quartile. As the number of businesses increase in an area (popularity quartile go up / looking down a column), the average rating of businesses go down. What's interesting is that if we look across each row (left to right), the rating is increasing monotonically (except for the first row where it stays flat from Q1 to Q3 before it increases in Q4). This means there's a positive correlation between number of reviews and average rating, and it holds true across all popularity quartile as well. This indicate businesses with more reviews typically end up having a higher average rating.
...
- **Test statistic**: The mean average rating of businesses with a zip popularity above the median, minus the mean average rating of businesses below the median. This is a directional test statistic using difference in mean. It needs to be directional as my hypothesis is claiming more popular area have **higher** review, not just simply differ.
...
Out of curiosity, I ran a tail test in the opposite direction (left tail), and it give a p-value of \(p < 0.001\), which is highly statistically significant. The data does seems to indicate a statistically significant difference exist, just in the opposite direction as my alternative hypothesis expect. This means while my theory is not supported by the evidence, a correlation exist and could be helpful for the regression model later on.
...
**Response variable**: I chose `avg_rating` because it is the variable my hypothesis was focused on. Although the test did not support my original hypothesis, the exploratory analysis found that zip popularity, review count, and price are all associated with average rating, so I want to see how well these can predict it. I also use `category`, which is another piece of metadata about the business the model can learn from, and is directly related to my original competition theory (since businesses of different category might not directly compete with each other).
...
| --- | --- | --- |
| `num_of_reviews` | Quantitative | Passed through untransformed for the baseline. Will log transform later. |
| `zip_popularity` | Quantitative | Passed through untransformed for the base line. Will log transform later. |
| `price_num` | Ordinal | I kept it numerical so the model only needs one coefficient for it, while respecting the ordering (`$` < `$$` < `$$$` < `$$$$`), and set missing values to 0 so the price term drops out entirely for those rows. |
| `price_missing` | Nominal (binary) | 1 when `price` is missing. Since the price term is 0 for those rows, this indicator effectively allows the model to assign a baseline for businesses with missing price instead of forcing a change in the coefficient of price, preventing missingness from distorting the relationship. |
...
The model already seems to be overfitting, with the train RMSE being almost 0.06 lower than the test RMSE. This is likely attributed to the one-hot encoded category column. Since each category becomes a separate features, and some features might have only a handful of businesses, this give the model a lot of opportunities to memorize the dataset in full. If the category doesn't provide information that is actually helpful, it'll only make the overfitting worse and increase the model variance. The final model addresses both problems.
...
- **`min_count`** on the category encoder, over `[1, 3, 5, 10, 25]`. This controls how aggressively rare categories are dropped, which will also help mitigate overfitting by significantly reducing the number of category features. A value of 1 keeps every category, including those appearing in only one business.

I ran the search on the training set only, so the test set stayed unseen. `GridSearchCV` then refits the best model on the full training set before evaluating on the test set. This is what get selected by the search:

**Selected values: `alpha = 10` and `min_count = 1`**, keeping all 2,127 category features.

The fact that it picked a `min_count` of 1 is surprising, but make sense when I think about it in hindsight. If a category only appeared for one row, then including it only impact that one row since the column is otherwise 0 for all other rows. That means, more of these features that only apply to one row can only help the model, not hurt it. By picking a `min_count` of 1, the model is effectively trying to memorize the entire dataset as much as possible. I compared it against a version with `min_count` of 3. It turns out `min_count = 1` only improved the train and test RMSE marginally by ~0.0005. The fact that the testing RMSE also improved is surprising and I cannot explain why. The model is clearly overfitting to noise, but the noise is harmless and sometimes even helpful for the model, even though the model now carries a lot of extra categories that probably will not appear anywhere else. My suspicion is that the test RMSE might of gotten lowered purely by chance because introducing new variable changes all the other coefficient. By chance, the new coefficient can happens to fit to the test set's noise better. A difference of 0.0005 is extremely low that it is can also just be random noise between trainings. For the purpose of predicting truly unseen data outside of this set, I think a higher `min_count` such as a value of 10 would make more sense and likely perform better, since there is no reason to expect why memorizing a feature that only apply to 1 business will help a future prediction. This could be a future area of study.
...
To assess whether the model perform worse for certain type of businesses, I chose the following groups and ran a permutation test.
...
I chose this split because the univariate analysis shows that businesses with few reviews have noisy and often extreme average ratings, including the large spike at exactly 5.0 stars along with the other whole-number rating spikes. The model is most likely to perform worse on businesses with fewer reviews, which are also what tends to be inputed to the model for a practical use case (such as predicting rating of businesses from a different location than Hawaii).
